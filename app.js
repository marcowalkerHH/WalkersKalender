...
function updateTheme() {
  ...
  document.querySelectorAll('.calendar button').forEach((door) => {
    door.style.setProperty('--door-border', user.accent);
  });
  updateDoorIcons();
}
...
function updateDoors() {
  ...
  document.querySelectorAll('.calendar button').forEach((door) => {
    ...
    const locked = state.mode === 'advent' && day > limit;
...
function shuffleDoorColors() {
  if (!calendar) return;
  const buttons = calendar.querySelectorAll('button');
  if (!buttons.length) return;
  const offset = Math.floor(Math.random() * 360);
  buttons.forEach((door, index) => {
    const hue = (offset + index * 17) % 360;
    const primary = `hsla(${hue}, 72%, 42%, 0.95)`;
    const secondary = `hsla(${(hue + 25) % 360}, 78%, 55%, 0.95)`;
    door.style.setProperty('--door-color', primary);
    door.style.setProperty('--door-color-secondary', secondary);
  });
  calendar.classList.add('shuffling');
  setTimeout(() => {
    calendar.classList.remove('shuffling');
  }, 900);
}
